...
【イレギュラーソーセージのルール】
以下のソーセージの場合は、1タルあたりの分量が異なります。
1. ビストロ: 肉のみ 2.0kg、トマト氷用 3.0kg
2. 玄米ソーセージ・サラミ塩: 肉のみ 1.5kg、トマト氷用 1.5kg
3. パリソー: 肉のみ 3.0kg、トマト氷用 4.0kg

【肉のみ用のパック分けルール】
肉のみの赤肉は、以下のパックサイズで分けられます。
- 6kgパック（基本ルールの場合は4タル分に相当）
- 3kgパック（基本ルールの場合は2タル分に相当）
- 端数は「余り肉」として扱います。

【トマト氷用のパック分けルール】
トマト氷用の赤肉は、以下のパックサイズで分けられます。（分量は基本ルールの1.8kgの倍率で計算してください）
